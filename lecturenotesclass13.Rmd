...
Not sure, but I guess around 20?
...
the mean is with a 95% confidence level
...
What does this mean?
Confidence level of 95% of mice on treatment that died
...
boot_prop <- mice %>% 
  filter(treat == 0) %>% 
  mutate(not_died = if_else(died == 1, "yes", "no")) %>% 
...
```{r practice_2_ci}
ci_mice_not_died <- get_ci(boot_prop, level = .90)
ci_mice_died
```
...
I am not sure if I did the exercise correctly, but I can assume from the prompt and from the data that it is the inverse of the previous dataset of mice that died.
